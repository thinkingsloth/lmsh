...
"""
lmshell - LLM-powered shell command generator
...
def show_first_run_warning():
    ...
    flag_file = config_dir / 'lmshell' / '.warning_shown'
...
By continuing to use lmshell, you acknowledge and accept these risks.
...
def show_help():
    ...
    help_text = """lmshell - LLM-powered command/script generator

USAGE:
    lmshell [OPTIONS] <query...>

OPTIONS:
    --base-url=<url>      OpenAI API base URL
                          (default: $LMSHELL_BASE_URL or http://127.0.0.1:7980/v1)
    ...
    --allow-sudo          Allow generation of commands requiring sudo/root privileges
                          (default: $LMSHELL_ALLOW_SUDO or false)
    --version             Show version information
    --help                Show this help message

ENVIRONMENT VARIABLES:
    LMSHELL_BASE_URL      Base URL for OpenAI-compatible API
    LMSHELL_API_TOKEN     API authentication token
    LMSHELL_MODEL_ID      Model ID to use for generation
    LMSHELL_OUTPUT        Default output format (bash, python, node, etc.)
    LMSHELL_ALLOW_SUDO    Allow sudo commands (true/false, default: false)

EXAMPLES:
    # Basic shell commands (uses current shell by default)
    lmshell find all python files modified today
    lmshell count lines in all txt files

    # Force Python output
    lmshell --output=python read json file and print all keys

    # Use different output formats
    lmshell --output=zsh create an associative array
...
lmshell will generate the appropriate command or script.
"""
    print(help_text)
    sys.exit(0)
...
def parse_args():
    ...
    base_url = os.getenv('LMSHELL_BASE_URL', DEFAULT_BASE_URL)
    api_token = os.getenv('LMSHELL_API_TOKEN')
    model_id = os.getenv('LMSHELL_MODEL_ID')
    output_type = os.getenv('LMSHELL_OUTPUT', current_shell)
    allow_sudo_env = os.getenv('LMSHELL_ALLOW_SUDO', 'false').lower()
    ...
    if not api_token:
        print("Error: LMSHELL_API_TOKEN not set", file=sys.stderr)
        sys.exit(1)

    if not model_id:
        print("Error: LMSHELL_MODEL_ID not set", file=sys.stderr)
        sys.exit(1)
